  ...
  *charge_busy = 1;
  ...
  if( FD_LIKELY( ctx->execle_idle_bitset ) ) { /* Optimize for schedule */
    ...
    switch( ctx->strategy ) {
      ...
      case FD_PACK_STRATEGY_PEBBLE:
        ...
        if( FD_UNLIKELY( pacing_execle_cnt >= (int) execle_cnt &&
          ...
          max_arrival_time_ns = ctx->auction_idx == ctx->auction_idx_last && ctx->leader_slot%FD_EPOCH_SLOTS_PER_ROTATION == FD_EPOCH_SLOTS_PER_ROTATION-1 ? LONG_MAX : ctx->auction_start_ns;
        } else {
          ...
          if ( FD_UNLIKELY( ctx->auction_period_ns == 0 ) ) {
            ...
            if( FD_UNLIKELY( now_ns > ctx->next_auction_ns) ) {
              ...
              if( FD_UNLIKELY( fd_pack_current_consumed(ctx->pack) >= ctx->auction_end_cu )) {
                ...
                flags |= fd_int_if( i < fd_int_max(3, pacing_execle_cnt), FD_PACK_SCHEDULE_TXN, 0);
                max_arrival_time_ns = ctx->auction_idx == ctx->auction_idx_last && ctx->leader_slot%FD_EPOCH_SLOTS_PER_ROTATION == FD_EPOCH_SLOTS_PER_ROTATION-1 ? LONG_MAX : now_ns;
                FD_LOG_INFO(( "auction %ld start: target %lu cu", ctx->auction_idx, ctx->auction_end_cu ));
              }
            } else {
              max_arrival_time_ns = LONG_MAX;
    ...
    if( FD_UNLIKELY( schedule_res.auction_stop && ctx->auction_start_ns
      && ( ctx->auction_idx < ctx->auction_idx_last || ctx->leader_slot % FD_EPOCH_SLOTS_PER_ROTATION != FD_EPOCH_SLOTS_PER_ROTATION - 1 ) ) ) {
